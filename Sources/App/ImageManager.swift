    ...
    func loadSaved() {
        ...
        for item in photos where item.isVisible {
            if !item.spaceImageFilenames.isEmpty {
                ...
                } else if let first = urls.first, let content = PhotoContent.load(from: first) {
                    createWindow(for: item, content: content)
                }
                ...
                guard let content = PhotoContent.load(from: imageURL) else { continue }
                createWindow(for: item, content: content)
            }
        }
    }
    ...
    func toggleVisibility(_ id: UUID) {
        ...
        if photos[index].isVisible {
            ...
            if !item.spaceImageFilenames.isEmpty {
                ...
                setupRotationTimer(for: item)
            } else if let content = PhotoContent.load(from: storageDir.appendingPathComponent(item.filename)) {
                createWindow(for: item, content: content)
            }
        } else {
            windows[id]?.hidePhoto()
